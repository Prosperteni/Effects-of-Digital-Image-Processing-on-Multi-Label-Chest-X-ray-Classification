...
def apply_gaussian_filter(image, kernel=5, sigma=1.0):
    if isinstance(image, Image.Image):
        image = np.array(image)
...
def apply_filter(image, filter_type, params):
    if filter_type == "clahe":
        ...
        return apply_gaussian_filter(image, params.get('kernel', 5), params.get('sigma', 1.0))
    elif filter_type == "bilateral":
        return apply_bilateral_filter(image, params.get('d', 9), params.get('sigma_color', 75), params.get('sigma_space', 75))
...
def run_experiment(config):
    """Run a single training experiment with given filter configuration"""
