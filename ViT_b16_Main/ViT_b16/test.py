"""
ViT-Base-16 (Vision Transformer) Testing for NIH Chest X-ray
Test script for trained ViT-B/16 model
"""
...
MODEL_PATH = "best_vit_nih.pth"      # Path to trained ViT model
...
def create_vit_model(model_name, num_classes=14):
    """Create ViT-Base-16 model (same architecture as training)"""
...
def plot_roc_curves(all_labels, all_preds, save_path='test_results/roc_curves_vit.png'):
    ...
    plt.title('ROC Curves - ViT-Base-16 on NIH Chest X-ray', fontsize=14)
...
def plot_confusion_matrix_heatmap(all_labels, all_preds, save_path='test_results/confusion_matrix_vit.png'):
    ...
    plt.title('Normalized Confusion Matrix - Per Disease (ViT-Base-16)', fontsize=14)
...
def plot_performance_bars(results_df, save_path='test_results/performance_bars_vit.png'):
    ...
    plt.title('Per-class AUC - ViT-Base-16 on NIH Test Set', fontsize=14)
...
def plot_predictions_grid(all_labels, all_preds, all_img_names, num_samples=20, save_path='test_results/predictions_grid_vit.png'):
    np.random.seed(42)
    ...
    plt.suptitle('Sample Predictions - ViT-Base-16', fontsize=16)
    plt.tight_layout()
    plt.savefig(save_path, dpi=150)
    plt.close()
...
if __name__ == '__main__':
    print("="*60)
    print("VIT-BASE-16 TRANSFORMER TESTING")
    print("="*60)
    ...
    print(f"\nLoading ViT-Base-16 model: {MODEL_NAME}")
    model = create_vit_model(MODEL_NAME, num_classes=NUM_CLASSES)
    ...
    results_df.to_csv('test_results_vit.csv', index=False)
    print("\n✅ Results saved to: test_results_vit.csv")
    ...
    detailed_df.to_csv('test_predictions_vit.csv', index=False)
    print("✅ Detailed predictions saved to: test_predictions_vit.csv")
    ...
    print("\n✅ Testing complete!")
    print("📁 Output files:")
    print("   - test_results_vit.csv")
    print("   - test_predictions_vit.csv")
    print("   - test_results/roc_curves_vit.png")
    print("   - test_results/confusion_matrix_vit.png")
    print("   - test_results/performance_bars_vit.png")
    print("   - test_results/predictions_grid_vit.png")