...
MODEL_PATH = "best_densenet_nih.pth"
BATCH_SIZE = 256  # Increased from 32 for faster testing (same as training)
IMG_SIZE = 224
NUM_CLASSES = 14
...
def create_densenet_model():
    """Create DenseNet-121 model with same architecture as training"""
...
def evaluate_test(model, loader, criterion):
    """Evaluate model on test set and return detailed metrics with optimized inference"""
    model.eval()
    total_loss = 0
    all_preds = []
    all_labels = []
...
def plot_roc_curves(all_labels, all_preds, save_path='test_results/roc_curves_densenet.png'):
    ...
    plt.title('ROC Curves - DenseNet-121 on NIH Chest X-ray', fontsize=14)
...
def plot_confusion_matrix_heatmap(all_labels, all_preds, save_path='test_results/confusion_matrix_densenet.png'):
    """Plot confusion matrix heatmap"""
    pred_binary = (all_preds > 0.5).astype(int)
    
    # Calculate per-class metrics for heatmap
    cm_data = []
    ...
    plt.title('Normalized Confusion Matrix - Per Disease', fontsize=14)
...
def plot_performance_bars(results_df, save_path='test_results/performance_bars_densenet.png'):
    ...
    bars = plt.bar(diseases, auc_scores, color=colors, alpha=0.7)
    ...
    plt.title('Per-class AUC - DenseNet-121 on NIH Test Set', fontsize=14)
...
def plot_predictions_grid(all_labels, all_preds, all_img_names, num_samples=20, save_path='test_results/predictions_grid.png'):
    ...
    plt.suptitle('Sample Predictions - DenseNet-121', fontsize=16)
    plt.tight_layout()
    plt.savefig(save_path, dpi=150)
    plt.close()
...
if __name__ == '__main__':
    print("="*60)
    print("DENSENET-121 TESTING (OPTIMIZED)")
    print("="*60)
    
    # Check if test file exists, otherwise use a split from train_val
    if os.path.exists(TEST_FILE):
        print("\nLoading test data from test_list_NIH.txt...")
        with open(TEST_FILE, 'r') as f:
            test_files = [line.strip() for line in f.readlines()]
    else:
        print("\n⚠️ Test file not found. Creating test split from CSV...")
        # Alternative: Use a random 20% split
    ...
    if 'model_state_dict' in checkpoint:
        ...
        print(f"  Training validation AUC was: {val_auc_info:.4f}" if isinstance(val_auc_info, (int, float)) else f"  Training validation AUC was: {val_auc_info}")
    else:
    ...
    # Save results to CSV
    results_df.to_csv('test_results_densenet.csv', index=False)
    print("\n✅ Results saved to: test_results_densenet.csv")
    ...
    # Calculate diseases above 0.7 AUC
    ...
    print("\n✅ Testing complete!")
    print("📁 Output files:")
    print("   - test_results_densenet.csv")
    print("   - test_results/roc_curves_densenet.png")
    print("   - test_results/confusion_matrix_densenet.png")
    print("   - test_results/performance_bars_densenet.png")
    print("   - test_results/predictions_grid.png")