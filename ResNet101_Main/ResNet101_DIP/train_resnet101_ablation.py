...
BATCH_SIZE = 128                    # ResNet-101 can handle 128
EPOCHS = 100
LEARNING_RATE = 1e-4
IMG_SIZE = 224
NUM_CLASSES = 14
...
def run_experiment(config):
    ...
    model = create_resnet101_model(NUM_CLASSES)
    model = model.to(DEVICE)
    print(f"ResNet-101 parameters: {sum(p.numel() for p in model.parameters())/1e6:.2f}M")
