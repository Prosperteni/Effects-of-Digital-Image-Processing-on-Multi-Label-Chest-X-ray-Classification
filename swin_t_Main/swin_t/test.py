...
MODEL_PATH = "best_swin_nih.pth"     # Path to trained Swin model
MODEL_NAME = "swin_tiny_patch4_window7_224"  # Must match training
...
# ==================== MODEL CREATION ====================
def create_swin_model(model_name, num_classes=14):
    """Create Swin Transformer model (same architecture as training)"""
...
def plot_roc_curves(all_labels, all_preds, save_path='test_results/roc_curves_swin.png'):
    ...
    plt.title('ROC Curves - Swin Transformer on NIH Chest X-ray', fontsize=14)
...
def plot_confusion_matrix_heatmap(all_labels, all_preds, save_path='test_results/confusion_matrix_swin.png'):
    ...
    plt.title('Normalized Confusion Matrix - Per Disease (Swin Transformer)', fontsize=14)
...
def plot_performance_bars(results_df, save_path='test_results/performance_bars_swin.png'):
    ...
    plt.title('Per-class AUC - Swin Transformer on NIH Test Set', fontsize=14)
...
def plot_predictions_grid(all_labels, all_preds, all_img_names, num_samples=20, save_path='test_results/predictions_grid_swin.png'):
    ...
    plt.suptitle('Sample Predictions - Swin Transformer', fontsize=16)
    plt.tight_layout()
    plt.savefig(save_path, dpi=150)
    plt.close()
...
if __name__ == '__main__':
    print("="*60)
    print("SWIN TRANSFORMER TESTING")
    print("="*60)
    ...
    # Load model
    print(f"\nLoading Swin Transformer model: {MODEL_NAME}")
    model = create_swin_model(MODEL_NAME, num_classes=NUM_CLASSES)
    
    checkpoint = torch.load(MODEL_PATH, map_location=DEVICE)
    
    # Handle checkpoint that contains model_state_dict
    ...
    # Save results to CSV
    results_df.to_csv('test_results_swin.csv', index=False)
    print("\n✅ Results saved to: test_results_swin.csv")
    ...
    detailed_df.to_csv('test_predictions_swin.csv', index=False)
    print("✅ Detailed predictions saved to: test_predictions_swin.csv")
    ...
    print("\n✅ Testing complete!")
    print("📁 Output files:")
    print("   - test_results_swin.csv")
    print("   - test_predictions_swin.csv")
    print("   - test_results/roc_curves_swin.png")
    print("   - test_results/confusion_matrix_swin.png")
    print("   - test_results/performance_bars_swin.png")
    print("   - test_results/predictions_grid_swin.png")